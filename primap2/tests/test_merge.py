...
# rewriting tests.utils.allclose to handle sparse arrays
...
def test_merge_fail_tolerance(opulent_sparse_coo_ds):
    da_start = opulent_sparse_coo_ds["CO2"]
    data_to_modify = opulent_sparse_coo_ds["CO2"].pr.loc[{"area": ["ARG"]}].pr.sum("area")
...
def test_merge_fail_tolerance_warn(opulent_sparse_coo_ds, caplog):
    da_start = opulent_sparse_coo_ds["CO2"]
    data_to_modify = opulent_sparse_coo_ds["CO2"].pr.loc[{"area": ["ARG"]}].pr.sum("area")
    data_to_modify.data = data_to_modify.data * 1.09
    da_merge = opulent_sparse_coo_ds["CO2"].pr.set(
...
def test_coords_not_matching_ds(opulent_sparse_coo_ds):
    ds_start = opulent_sparse_coo_ds
    ds_merge = opulent_sparse_coo_ds.rename({"time": "year"})

    with pytest.raises(ValueError, match="pr.merge error: coords of objects to merge must agree"):
        ds_start.pr.merge(ds_merge)


def test_coords_not_matching_da(opulent_sparse_coo_ds):
    da_start = opulent_sparse_coo_ds["CH4"]
    da_merge = opulent_sparse_coo_ds["CH4"].rename({"time": "year"})

    with pytest.raises(ValueError, match="pr.merge error: coords of objects to merge must agree"):
        da_start.pr.merge(da_merge)


def test_dims_not_matching_ds(opulent_sparse_coo_ds):
    ds_start = opulent_sparse_coo_ds
    ds_merge = opulent_sparse_coo_ds.rename_dims({"time": "year"})

    with pytest.raises(ValueError, match="pr.merge error: dims of objects to merge must agree"):
        ds_start.pr.merge(ds_merge)


def test_dims_not_matching_da(opulent_sparse_coo_ds):
    da_start = opulent_sparse_coo_ds["CH4"]
    da_merge = opulent_sparse_coo_ds["CH4"].swap_dims({"time": "year"})
...
def test_merge_ds_add_coord(opulent_sparse_coo_ds):
    country_names = ["Colombia", "Argentina", "Mexico", "Bolivia"]
    ds_full = opulent_sparse_coo_ds.assign_coords(country_name=("area (ISO3)", country_names))
    sel = {"time": slice("2000", "2005"), "scenario": "highpop", "source": "RAND2020"}
...
def test_merge_ds_add_coord_tolerance(opulent_sparse_coo_ds):
    country_names = ["Colombia", "Argentina", "Mexico", "Bolivia"]
    ds_full = opulent_sparse_coo_ds.assign_coords(country_name=("area (ISO3)", country_names))
    sel = {"time": slice("2000", "2005"), "scenario": "highpop", "source": "RAND2020"}
    ds = ds_full.pr.loc[sel]
...
def test_merge_ds_add_coord_disjoint_vars(opulent_sparse_coo_ds):
    country_names = ["Colombia", "Argentina", "Mexico", "Bolivia"]
    ds_full = opulent_sparse_coo_ds.assign_coords(country_name=("area (ISO3)", country_names))
...
def test_merge_message_time_yearly(opulent_sparse_coo_ds):
    da_start: xr.DataArray = opulent_sparse_coo_ds["CO2"]
    npa = opulent_sparse_coo_ds["CO2"].loc[{"time": "2000"}].to_numpy().copy()
    npa[:] = 0.0
    da_merge: xr.DataArray = opulent_sparse_coo_ds["CO2"].pr.set(
...
def test_merge_message_time_daily(opulent_sparse_coo_ds):
    da_start: xr.DataArray = opulent_sparse_coo_ds["CO2"]
...
def test_log_formatting(opulent_sparse_coo_ds, caplog):
    da_start = opulent_sparse_coo_ds["CO2"]
    data_to_modify = (
        opulent_sparse_coo_ds["CO2"]
        .pr.loc[{"area": ["ARG"], "time": ["2001", "2002"]}]
        .pr.sum("area")
    )
    data_to_modify.data = data_to_modify.data * 1.09
    da_merge = opulent_sparse_coo_ds["CO2"].pr.set(
...
def test_log_formatting_single_date(opulent_sparse_coo_ds, caplog):
    da_start = opulent_sparse_coo_ds["CO2"]
    data_to_modify = (
        opulent_sparse_coo_ds["CO2"].pr.loc[{"area": ["ARG"], "time": ["2000"]}].pr.sum("area")
    )
    data_to_modify.data = data_to_modify.data * 1.09
    da_merge = opulent_sparse_coo_ds["CO2"].pr.set(
        "area", "ARG", data_to_modify, existing="overwrite"
