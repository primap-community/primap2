...
class TestToNetCDF:
    ...
    def test_processing_info_missing(
        self, opulent_processing_ds: xr.Dataset, caplog, tmp_path, missing
    ):
        """Processing info can be missing for individual timeseries.

        This happens for example when a dataset generated by the csg uses different
        categories for different variables. Depending on how the dataset was built,
        the missing information is marked by ``None`` or by NaN.
        """
        ds = opulent_processing_ds
        ds["Processing of CO2"].loc[{"area (ISO3)": ["ARG", "MEX", "BOL"]}] = missing
        caplog.set_level(logging.INFO)
        ...
        nds.pr.ensure_valid()
        assert not caplog.records
        # missing processing info stays missing, the rest is read back unchanged
        processing = nds["Processing of CO2"]
        assert processing.pr.loc[{"area": ["ARG", "MEX", "BOL"]}].isnull().all()
        assert processing.pr.loc[{"area": ["COL"]}].notnull().all()
        xr.testing.assert_identical(ds, nds)

    def test_processing_info_all_missing(self, opulent_processing_ds: xr.Dataset, tmp_path):
        """A variable without any processing info at all must round-trip as well."""
        ds = opulent_processing_ds
        ds["Processing of CO2"][:] = None
