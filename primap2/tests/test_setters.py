...
class TestDASetter:
    ...
    def test_exists_empty_default(self, da: xr.DataArray, ts: np.ndarray, co2: pint.Unit, new):
    ...
    def test_exists_somena_default(self, da: xr.DataArray, ts: np.ndarray, co2: pint.Unit, new):
        da.loc[{"area (ISO3)": "COL"}].pint.magnitude[:] = np.nan
    ...
    def test_exists_overwrite(self, da: xr.DataArray, ts: np.ndarray, co2: pint.Unit, new):
    ...
    def test_new_from_array(self, da: xr.DataArray, ts: np.ndarray, co2: pint.Unit):
        ...
        actual_error = da.pr.set("area", "CUB", 2 * da.pr.loc[{"area": "COL"}], existing="error")
    ...
    def test_multidimensional_data_array(self, mda: xr.DataArray):
        actual = mda.pr.set("a", "a4", xr.DataArray(np.ones((4, 5)), coords=[mda["b"], mda["c"]]))
        expected = mda.reindex({"a": ["a1", "a2", "a3", "a4"]}).fillna(1)
        assert_aligned_equal(actual, expected)
    ...
    def test_automatic_unit_conversion(self, da: xr.DataArray, ts: np.ndarray, co2, new):
        actual = da.pr.set("area", "COL", ts * ureg("Mg CO2 / year"), existing="overwrite", **new)
        expected = da
...
class TestDsSetter:
    ...
    def test_existing_default(self, minimal_ds: xr.Dataset, new):
        with pytest.raises(ValueError):
            minimal_ds.pr.set("area", "COL", minimal_ds.pr.loc[{"area": "COL"}] * 2, **new)
